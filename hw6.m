...
for k=1:length(t)-1                   % z contains past inputs
  ...
  (lpfs*cos(theta(k))-lpfs*sin(theta(k))) * (lpfc*cos(theta(k))+ lpfc*sin(theta(k)));
  theta(k+1)=theta(k)-mu*(lpfs*cos(theta(k))-lpfs*sin(theta(k))) * (lpfc*cos(theta(k))+ lpfc*sin(theta(k)));   % algorithm update
end
